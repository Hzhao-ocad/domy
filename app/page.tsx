...
import { activateDome, advanceLighting, clearActivation, createLightingState, type LightingBehavior } from '@/lib/lighting';
import { setPaletteColor } from '@/lib/palette';
import { nearestPointWithinDistance, offsetControlPoints } from '@/lib/path';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    torso.position.z = .68; head.position.z = 1.18; neck.position.z = 1.01; neck.rotation.x = Math.PI / 2; leftLeg.position.set(-.09, 0, .2); rightLeg.position.set(.09, 0, .2); leftArm.position.set(-.23, 0, .69); rightArm.position.set(.23, 0, .69); leftArm.rotation.y = .35; rightArm.rotation.y = -.35; pedestrian.add(torso, head, neck, leftLeg, rightLeg, leftArm, rightArm); pedestrian.traverse((object) => { if (object instanceof THREE.Mesh) { object.castShadow = true; object.receiveShadow = true; } }); pedestrian.visible = false; scene.add(pedestrian);
    ...
    let lighting = createLightingState(values.current.count, { propagationDelay: values.current.delay, propagationFactor: values.current.factor, decayInterval: values.current.decayStep, decayAmount: values.current.decay }, values.current.behavior);
    ...
    const positionPedestrian = (index: number, shouldActivate = false) => {
    ...
    const panel = host.parentElement!.querySelector('.panel')!, hudButton = document.createElement('button'); panel.classList.add('hud-hidden'); hudButton.className = 'hud-toggle'; hudButton.textContent = 'Show HUD'; const toggleHud = () => { const hidden = panel.classList.toggle('hud-hidden'); hudButton.textContent = hidden ? 'Show HUD' : 'Hide HUD'; }; hudButton.addEventListener('click', toggleHud); host.parentElement!.appendChild(hudButton);
    ...
    const hidePedestrian = () => { pedestrian.visible = false; if (active !== null) { lighting = clearActivation(lighting, performance.now()); active = null; } };
    const hover = (e: PointerEvent) => { if (dragging !== null || orbiting) return; const point = groundPoint(e); if (!point) return; const samples = pedestrianSamples(); const index = nearestPointWithinDistance(samples.map((sample) => ({ x: sample.x, y: sample.y })), { x: point.x, y: point.y }, values.current.activationDistance); if (index === null) { hidePedestrian(); return; } pedestrian.visible = true; positionPedestrian(index, true); };
    ...
    renderer.domElement.addEventListener('pointerdown', down); renderer.domElement.addEventListener('pointermove', move); renderer.domElement.addEventListener('pointerup', up); renderer.domElement.addEventListener('pointerleave', hidePedestrian);
    ...
    return () => { cancelAnimationFrame(frame); window.removeEventListener('resize', resize); renderer.domElement.removeEventListener('pointerdown', down); renderer.domElement.removeEventListener('pointermove', move); renderer.domElement.removeEventListener('pointerup', up); renderer.domElement.removeEventListener('pointerleave', hidePedestrian); hudButton.removeEventListener('click', toggleHud); hudButton.remove(); groundGeometry.dispose(); stoneGeometry.dispose(); stoneMaterial.dispose(); renderer.dispose(); host.removeChild(renderer.domElement); };
  }, []);
  useEffect(() => { api.current?.rebuild(); }, [params.count, params.radius, params.cut, params.miniMin, params.miniMax, params.miniScaleMin, params.miniScaleMax, params.points, params.pointLightsPerDome, params.behavior]);
